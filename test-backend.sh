...
echo "=================================="
echo ""
echo "Backend URL: https://basketballaiappapi.onrender.com"
echo ""
...
START_TIME=$(date +%s)
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 60 https://basketballaiappapi.onrender.com/health)
END_TIME=$(date +%s)
...
if [ "$HTTP_CODE" = "200" ]; then
    ...
    echo ""
    echo "📊 Backend Health:"
    curl -s https://basketballaiappapi.onrender.com/health | python3 -m json.tool 2>/dev/null || echo "Could not parse JSON"
    echo ""
